...
/*bool ValidarZombieAlEntrar(Zombie zombie) {
    if (zombie.nombre.empty()) {
        cout << "ERROR007: El nombre del zombie no puede estar vacío." << endl;
        return false;
    }
    ...
    if (zombie.inteligencia != "vida" && zombie.inteligencia != "resistencia" &&
        zombie.inteligencia != "velocidad" && zombie.inteligencia != "suerte" &&
        zombie.inteligencia != "nivel" && !zombie.inteligencia.empty()) {
        cout << "ERROR011: La inteligencia del zombie debe ser una de las condiciones válidas o estar vacía." << endl;
        return false;
    }
    return true;
}*/
...
bool BuscarEnLaMochilaDelSoldado(Soldado soldado, int seleccion){
    int respuesta;
